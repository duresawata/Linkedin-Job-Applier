...
class MyFirstGUI:
    ...
    def apply(self):
        bot = easyApply.ApplyLinkedin(email = self.emailEntry.get(), phone=self.phone.get(), username=self.linkedInUserNameEntry.get(), password=self.linkedInPass.get(), driverPath=self.driver_path.get(), jobTitle=self.jobTitle.get(),
